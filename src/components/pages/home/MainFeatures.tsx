...
export const FeaturesSection = () => {
  return (
    <section
      className="w-full py-6 lg:py-6 pb-6 bg-white flex flex-col items-center px-6 lg:px-20 relative z-50 overflow-visible"
    ...
    >
      <div className='max-w-[1400px] w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-12 items-start'>
        ...
        >
          ...
          <div aria-hidden="true" className='absolute inset-0 bg-blue-50/50 rounded-full blur-[120px] pointer-events-none translate-z-0' />
          ...
          {/* Floating stat cards intentionally moved below (to be attached to FeaturesSection root) */}
        
        </motion.div>

      </div>

      {/* Floating stat cards attached to FeaturesSection so they aren't clipped by inner stacking contexts */}
      <motion.div
        className='absolute -top-4 left-6 md:left-0 md:-top-10 z-[9980] bg-white/95 backdrop-blur-md rounded-[24px] p-6 shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-white w-[220px] md:w-[320px] will-change-transform'
      ...
      <motion.div
        className='absolute -bottom-6 right-6 md:right-[-8] md:-bottom-16 z-[9999] bg-[#2D1B69] rounded-[24px] p-6 w-[200px] md:w-[300px] will-change-transform'
        initial={{ opacity: 0, scale: 0.8 }}
        whileInView={{ opacity: 1, scale: 1 }}
